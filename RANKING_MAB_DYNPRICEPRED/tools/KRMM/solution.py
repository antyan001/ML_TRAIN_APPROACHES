...
def avg_emb(toks):
    ...
    unk_embedding = np.random.uniform(-0.2, 0.2, dim_of_embed) #emb_matrix["weight"][1].numpy() 
...
def send_json():
    ...
    BASE_IP = 'http://localhost:11000/'
    INDEX_CREATE_TIMEOUT = 300

    with open("./jl/documents.json", "r") as f:
        json_payload = json.load(f)    

    responce = requests.post(BASE_IP + 'update_index', json=json_payload, timeout=INDEX_CREATE_TIMEOUT)
    
    return jsonify({"json_status": responce.status_code, "create_index": json.loads(responce.text)})
