...
class SparkHelper:
    ...
    def create_sql_context(self, config=None, name='None', port='4440', instances=5, n_cores=2, spark_version='2.1', kernel='PYENV.ZNO0059623792'):
        ...
        os.environ['PYSPARK_PYTHON'] = '/opt/cloudera/parcels/{}/bin/python'.format(kernel)
        sys.path.insert(0, os.path.join (spark_home,'python'))
        sys.path.insert(0, os.path.join (spark_home,'python/lib/py4j-0.10.7-src.zip'))
        #os.environ['SPARK_HOME'] = spark_home
        #os.environ['LD_LIBRARY_PATH'] = '/opt/cloudera/parcels/{}/usr/lib/oracle/12.2/client64/lib'.format(kernel)
        os.environ['PYTHONPATH']='/home/patrakova1-es_ca-sbrf-ru/.local/lib/python3.5/site-packages/'
        ...
        if config is None:
            ...
            config.setAll(
                [
                   ...
                   #('spark.driver.extraClassPath', '/opt/cloudera/parcels/{}/usr/lib/oracle/12.2/client64/lib/ojdbc8.jar'.format(kernel)),
                   #('spark.executor.extraClassPath', '/opt/cloudera/parcels/{}/usr/lib/oracle/12.2/client64/lib/ojdbc8.jar'.format(kernel))
                ])
    ...
    def create_table_from_select(self, db_in='t_ural_kb', table_name='', db_out='t_ural_kb', prefix='0_'):
        ...
        self.sqlContext.sql('DROP TABLE IF EXISTS {db_out}.{tab}'.format(db_out=db_out, tab=prefix+table_name))
